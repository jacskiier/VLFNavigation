...
if __name__ == '__main__':
    ...
    assert np.linalg.norm(llhEstArray[0,:] - llh) < 1 and np.linalg.norm(llhEstArray[1,:] - llh2) < 1, "EcefToLlh failed to convert correctly {0} to the True:{1}".format(llhEst, llh)

    # test output array
    llhEstArray = EcefToLlh([ecef, ecef2], "fast")
    assert llhEstArray.shape[1] > 1, "EcefToLlh failed to output an array"
    assert np.linalg.norm(llhEstArray[0, :] - llh) < 1 and np.linalg.norm(llhEstArray[1, :] - llh2) < 1, "EcefToLlh failed to convert correctly {0} to the True:{1} or {2} to {3}".format(llhEstArray[0,:],
