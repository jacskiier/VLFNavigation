...
def test_rbm(learning_rate=0.1, training_epochs=15,
    ...
    :param dataset: the locatin of the dataset to use

    :param output_folder: the location to save the output images
    ...
    :param n_hidden: the number of hidden nodes in the rbm

    :param naturalHeight: the height shape of the input features

    :param naturalWidth: the width shape of the input features
    ...
    rbm = RBM(inputs=x, n_visible=inputs,
    ...
    image_data = numpy.zeros(
        ((naturalWidth + tileSpacingx) * n_samples + 1, (naturalHeight + tileSpacingy) * n_chains - 1),
        dtype='uint8'
    )
    for idx in xrange(n_samples):
        ...
        image_data[(naturalWidth + tileSpacingx) * idx:(naturalWidth + tileSpacingx) * idx + naturalWidth, :] = tile_raster_images(
            X=vis_mf,
            img_shape=(naturalWidth, naturalHeight),
